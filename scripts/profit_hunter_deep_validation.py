...
核心功能：
1. ✅ Reddit 痛点挖掘（搜索用户抱怨和真实需求）
2. ✅ Google SERP 分析（检测市场空白：论坛多但工具少）
3. ✅ 真实需求验证（多维度验证：讨论数、痛点信号、市场空白）
4. ✅ 深度评分系统（Reddit分×50% + SERP分×30% + 商业意图×20%）
5. ✅ HTML 报告生成（可视化验证结果）
...
VALIDATION_CONFIG = {
    ...
    "PAIN_KEYWORDS": [                # 痛点信号词
        ...
        "waste of time", "too slow", "annoying", "confusing"
    ],
    "VALIDATION_THRESHOLD": 3,        # 最少需要3个真实需求验证
    "MIN_VALIDATION_SCORE": 50,       # 最低验证分数
...
def search_reddit_pain_points(keyword: str) -> Dict:
    ...
    """
    log_execution(f"🔍 Reddit 验证: {keyword}")
    ...
    try:
        ...
        time.sleep(VALIDATION_CONFIG["DELAY_BETWEEN_REQUESTS"])  # 礼貌延迟
        
    except Exception as e:
        log_execution(f"⚠️ Reddit 验证失败: {str(e)[:100]}", "WARNING")
...
def analyze_google_serp(keyword: str) -> Dict:
    ...
    """
    log_execution(f"🔍 Google SERP 验证: {keyword}")
    ...
    try:
        ...
        log_execution(f"⚠️ SERP 验证失败: {str(e)[:100]}", "WARNING")
...
def deep_validate_keyword(keyword: str) -> Dict:
    ...
    if reddit_data["validation_score"] > 30:
        ...
        reasoning_points.append(f"✅ Reddit 有{reddit_data['total_mentions']}条讨论，"
                               f"{len(reddit_data['pain_signals'])}个痛点信号")
    else:
        reasoning_points.append(f"⚠️ Reddit 讨论较少({reddit_data['total_mentions']}条)")
    ...
    # 最终判断
    is_real_need = validation_score >= VALIDATION_CONFIG["MIN_VALIDATION_SCORE"]
...
<body>
    <div class="container">
        <div class="header">
            <h1>🎯 深度需求验证报告</h1>
            <p>基于 Reddit 痛点挖掘 + Google SERP 分析</p>
        ...
        html_content += f"""
            <div class="opportunity">
                ...
                <div class="evidence-box">
                    ...
                    • Reddit 讨论: {row['reddit_mentions']}条<br>
...
"""
    ...
    log_execution(f"📄 HTML 报告已生成: {output_path}")
    return output_path
...
def main():
    ...
    parser.add_argument('--input', type=str, required=True, help='输入 CSV 文件路径（包含 keyword 列）')
